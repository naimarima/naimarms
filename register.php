...
<label for="sel1">Select Student ID :</label><br>
					<select name="std_id" style="width:250px;height:30px;padding:5px 5px;">
									...
									while($row = mysqli_fetch_array($query))
														{
														?>
									<option value="<?php echo $row['std_id']; ?>"><?php echo $row['std_id'],",", $row['std_name'], ",", $row['batch_no'] ?></option>
									<?php } ?>
							  </select><br><br>
...
if(isset($_POST['send'])){
	...
	$sql = "insert into register(std_id,crs_id,) values ('$std_id',,'$crs_id')";
...
<?php
$sql = "select student.std_id, student.std_name, batch.batch_no, course.crs_id, course.crs_code, course.crs_title
from student,batch,course";
